...
export default function UploadPage() {
  ...
  // HeyRoya 13-column Metadata Correction Template (round-trips with HeyRoya
  // without schema translation). Preamble matches HeyRoya_Correction_Template
  // (3).xlsx exactly: title rows 1–3, blank row 4, instruction bullets 5–7,
  // header row 8. One row per work-line; the notes column flags any
  // validation issues found by Kataloghub. Publishers only edit the
  // 'corrected value' and 'notes' columns. Numeric identifiers (writer_ipi,
  // publisher_ipi) are prefixed with a leading apostrophe so Excel preserves
  // leading zeros.
  function downloadCsv() {
    if (!result) return;

    const seenDup: Record<string, true> = {};
    const rowNotes = (r: Row): string => {
      const notes: string[] = [];
      if (!r.work_id) notes.push("work_id saknas");
      if (!r.writer_ipi || r.writer_ipi.length < 9) {
        notes.push(`writer_ipi ${r.writer_ipi || "(tom)"} – Ogiltigt IPI-format`);
      }
      if (["C", "CA", "A"].indexOf(r.role_code) < 0) {
        notes.push(`role_code ${r.role_code || "(tom)"} – Ogiltig rollkod`);
      }
      const s = Number(r.share);
      if (!(s > 0) || s > 100) {
        notes.push(`share ${r.share || "(tom)"} – Ogiltig delning`);
      }
      if (["E", "SE", "AM"].indexOf(r.agreement_type) < 0) {
        notes.push(`agreement_type ${r.agreement_type || "(tom)"} – Ogiltig avtalstyp`);
      }
      const dupKey = (r.work_id || "") + "|" + (r.writer_ipi || "");
      if (r.work_id || r.writer_ipi) {
        if (seenDup[dupKey]) notes.push("Dubblett av work_id+writer_ipi");
        else seenDup[dupKey] = true;
      }
      return notes.join("; ");
    };
    ...
    const out: string[] = [
      "HeyRoya" + PAD,
      "Metadata Correction Template" + PAD,
      "For Music Publishers   ·   File-based correction workflow" + PAD,
      PAD,
      "•  Publishers only edit the 'corrected_value' and 'notes' columns." + PAD,
      "•  All other fields reflect the original metadata as received." + PAD,
      "•  Return the completed file to HeyRoya for processing." + PAD,
      "work id,title,writer name,writer ipi,role code,share percentage,publisher name,publisher ipi,STIM,iswc,isrc,corrected value,notes",
    ];

    for (const r of result.rows) {
      const writerIpi = r.writer_ipi ? "'" + r.writer_ipi : "";
      out.push(
        [
          r.work_id || "",
          r.title || "",
    ...
    downloadFile(out.join("\n"), "kataloghub-worksheet-" + result.scanId + ".csv", "text/csv");
  }
        ...
        <>
          <Card className="mt-4">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <a
                href={process.env.NEXT_PUBLIC_HEYROYA_UPLOAD_URL || "https://upload.heyroya.se/"}
                target="_blank"
              ...
              <button
                type="button"
                onClick={downloadPdf}
                className="ml-auto rounded border border-border px-4 py-2 text-sm text-text-muted hover:border-text-muted hover:text-text"
              >
