...
export default function UploadPage() {
  ...
  // 9-column worksheet shape matching the Kataloghub_Validation .xlsm template.
  // Numeric identifiers are prefixed with a leading apostrophe so Excel
  // preserves leading zeros.
  function downloadCsv() {
    if (!result) return;
    const NUMERIC_FIELDS: Record<string, 1> = { writer_ipi: 1, duplicate: 1, iswc: 1, isrc: 1 };
    const generated = new Date().toISOString().replace("T", " ").slice(0, 16);
    const out: string[] = [
      "# Worksheet generated by Kataloghub - " + generated,
      "# Publisher: [ENTER PUBLISHER]",
      "# Catalog: " + result.scanId,
      "# Numeric identifiers (writer_ipi, iswc, isrc) are quoted with a leading ' for Excel compatibility.",
      "Line_ID,work_id,field,original_value,corrected_value,decision_required,status,publisher_,notes",
    ];
    result.issues.forEach((x, i) => {
      const lineId = "L-" + String(i + 1).padStart(4, "0");
      const orig = x.original_value && NUMERIC_FIELDS[x.field] ? "'" + x.original_value : x.original_value;
      const note = x.status ? x.comment + " [" + x.status + "]" : x.comment;
      out.push([lineId, x.work_id || "", x.field, orig, "", "", "", "", note].map(csvEscape).join(","));
    });
